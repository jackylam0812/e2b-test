...
class SandboxCapacityTester:
    ...
    def extend_sandbox_lifetime(self, sandbox: Sandbox) -> bool:
        ...
        """
        try:
            # 使用 E2B SDK 的 set_timeout 方法来延长生命周期
            # 这个方法会自动处理认证，避免手动调用 API 时的 401 错误
            sandbox.set_timeout(self.timeout)
            logger.debug(f"沙箱 {sandbox.sandbox_id[:12]} 生命周期已延长到 {self.timeout}秒")
            return True

        except Exception as e:
            logger.warning(f"延长沙箱 {sandbox.sandbox_id[:12]} 生命周期失败: {e}")
            return False
