...
fio --name=randread --ioengine=libaio --iodepth=1 --rw=randread \
    --bs=4k --direct=1 --size=$TEST_SIZE --numjobs=1 --runtime=10 --time_based \
    --group_reporting --output-format=json > /tmp/fio_randread.json 2>&1
...
fio --name=seqread --ioengine=libaio --iodepth=16 --rw=read \
    --bs=1m --direct=1 --size=$SEQ_TEST_SIZE --numjobs=1 --runtime=10 --time_based \
    --group_reporting --output-format=json > /tmp/fio_seqread.json 2>&1
...
fio --name=randwrite --ioengine=libaio --iodepth=1 --rw=randwrite \
    --bs=4k --direct=1 --size=$TEST_SIZE --numjobs=1 --runtime=10 --time_based \
    --group_reporting --output-format=json > /tmp/fio_randwrite.json 2>&1
...
fio --name=seqwrite --ioengine=libaio --iodepth=16 --rw=write \
    --bs=1m --direct=1 --size=$SEQ_TEST_SIZE --numjobs=1 --runtime=10 --time_based \
    --group_reporting --output-format=json > /tmp/fio_seqwrite.json 2>&1
...
fio --name=readlat --ioengine=libaio --iodepth=1 --rw=randread \
    --bs=4k --direct=1 --size=$TEST_SIZE --numjobs=1 --runtime=10 --time_based \
    --group_reporting --output-format=json --lat_percentiles=1 > /tmp/fio_readlat.json 2>&1
...
fio --name=writelat --ioengine=libaio --iodepth=1 --rw=randwrite \
    --bs=4k --direct=1 --size=$TEST_SIZE --numjobs=1 --runtime=10 --time_based \
    --group_reporting --output-format=json --lat_percentiles=1 > /tmp/fio_writelat.json 2>&1
